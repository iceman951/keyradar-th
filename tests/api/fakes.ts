...
const toGameDto = (game: Game): GameDto => ({
  ...game,
  editions: game.editions.map((e) => ({ ...e }))
})
...
export const createFakeCatalog = (): RouteCatalogService => ({
  ...
  async listStores() {
    ...
    const order = new Map(
      sortStoresByTrust(stores).map((store, index) => [store.id, index])
    )
...
export const createFakePricing = (): PricingService => ({
  async getOffers(gameSlug, editionKey) {
    ...
    const sorted = offers
      .map((offer) => ({
        ...offer,
        isHistoricalLow: offer.id === historicalId
      }))
      .sort(
        (a, b) =>
          a.finalSatang - b.finalSatang || a.storeId.localeCompare(b.storeId)
      )
    ...
    return {
      ...
      offers: sorted.map((offer) => ({
        ...offer,
        observedAt: new Date(
          Date.now() - offer.updatedMinutesAgo * 60_000
        ).toISOString()
      })),
  ...
  async getPriceHistory(gameSlug, days) {
    ...
    const ending =
      bestThaiOffer(offers)?.finalSatang ?? firstEdition.steamPriceSatang
    return buildPriceHistory(gameSlug, days, ending, Date.now()).map(
      (point) => ({
