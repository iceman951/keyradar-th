...
describe('mock repository', () => {
  ...
  it('returns deterministic snapshots with valid integer-satang prices', async () => {
    ...
    expect(first.offers.map((offer) => offer.finalSatang)).toEqual(
      [...first.offers].sort((a, b) => a.finalSatang - b.finalSatang).map((offer) => offer.finalSatang)
    );
    ...
    expect(first.offers.every((offer) => new URL(offer.purchaseUrl).protocol === 'https:')).toBe(true);
  });
  ...
  it('provides a deterministic repository error mode', async () => {
    await expect(new MockGameRepository({ offerMode: 'error' }).getOffers('elden-ring'))
