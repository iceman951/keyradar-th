...
const FX = {
  ...
  base: 'USD',
  date: '2026-08-08',
  rates: { THB: 33.055, EUR: 0.86693, GBP: 0.74352 }
}
...
describe('mapDealToCatalogOffer', () => {
  ...
  it('adds an FX conversion fee for non-THB prices and keeps the total consistent', () => {
    ...
    expect(offer.feesSatang).toBe(
      Math.round(offer.advertisedSatang * FX_FEE_RATE)
    )
  ...
  it('charges no conversion fee when the store already quotes THB', () => {
    const offer = mapDealToCatalogOffer({
      ...
      deal: deal({
        ...
        price: { amount: 1590, amountInt: 159000, currency: 'THB' }
      })
    })!
  ...
  it('derives region from the curated table and defaults unknown stores to uncertain', () => {
    expect(
      mapDealToCatalogOffer({ ...base, storeId: 'steam', deal: deal() })!.region
    ).toBe('thailand')
    expect(
      mapDealToCatalogOffer({ ...base, storeId: 'dreamgame', deal: deal() })!
        .region
    ).toBe('sea')
    const unknown = mapDealToCatalogOffer({
      ...base,
      storeId: 'brand-new-shop',
      deal: deal()
    })!
  ...
  it('flags a historical low only at or below the recorded low', () => {
    const offer = mapDealToCatalogOffer({ ...base, deal: deal() })!
    const atLow = mapDealToCatalogOffer({
      ...base,
      deal: deal(),
      historyLowSatang: offer.advertisedSatang
    })!
    const above = mapDealToCatalogOffer({
      ...base,
      deal: deal(),
      historyLowSatang: 100
    })!
  ...
  it('labels DRM so the Steam filter matches exactly the Steam-activated stores', () => {
    ...
    expect(drmLabel(deal({ drm: [{ id: 1, name: 'GOG' }] }), 'gog')).toBe(
      'GOG.com'
    )
    expect(drmLabel(deal({ drm: [{ id: 2, name: 'EA App' }] }), 'gmg')).toBe(
      'EA app'
    )
    // filters.ts decides by substring, so non-Steam labels must not contain "steam".
    expect(
      drmLabel(deal({ drm: [{ id: 1, name: 'GOG' }] }), 'gog').toLowerCase()
    ).not.toContain('steam')
  })
})
...
describe('buildCatalog', () => {
  ...
  const prices = {
    'itad-std': {
      ...
      deals: [
        deal({
          shop: { id: 61, name: 'Steam' },
          price: { amount: 1290, amountInt: 129000, currency: 'THB' },
          regular: { amount: 1490, amountInt: 149000, currency: 'THB' }
        }),
        deal({ shop: { id: 6, name: 'Fanatical' } }),
        deal({
          shop: { id: 999, name: 'Kinguin' },
          url: 'https://kinguin.net/x'
        })
      ]
    }
  }
  ...
  const steamPrices = {
    4242: { initialSatang: 149000, finalSatang: 129000, discountPercent: 13 }
  }
  const input = {
    games: [game],
    stores,
    prices,
    idByEdition: { 'test-game|standard': 'itad-std' },
    steamPrices,
    rates: RATES,
  ...
  it('builds one lookup per edition because ITAD models editions as separate games', () => {
    ...
    expect(lookups.map((l) => l.lookupTitle)).toEqual([
      'Test Game',
      'Test Game Deluxe Edition'
    ])
  })

  it('excludes shops that are not in the fixture allowlist', () => {
    const catalog = buildCatalog(input)
    const ids = catalog.games['test-game'].editions.standard.offers.map(
      (o) => o.storeId
    )
  ...
  it('drops editions with no offers but keeps games that resolved at least one', () => {
    const catalog = buildCatalog(input)
    expect(Object.keys(catalog.games['test-game'].editions)).toEqual([
      'standard'
    ])
  })
  ...
  it('takes the Steam price from Steam, not from ITAD', () => {
    ...
    const steamOffer = standard.offers.find(
      (offer) => offer.storeId === 'steam'
    )!
    ...
    expect(steamOffer.purchaseUrl).toBe(
      'https://store.steampowered.com/app/4242/'
    )
  })
  ...
  it('falls back to the fixture price when Steam has nothing to say', () => {
    ...
    expect(standard.offers.some((offer) => offer.storeId === 'steam')).toBe(
      false
    )
  })
  ...
  it('carries the previous run forward and marks it stale when nothing resolves', () => {
    ...
    expect(
      carried.games['test-game'].editions.standard.offers.length
    ).toBeGreaterThan(0)
  })
  ...
  it('keeps every edition with empty offers when there is nothing to carry forward', () => {
    ...
    expect(Object.keys(empty.games['test-game'].editions).sort()).toEqual([
      'deluxe',
      'standard'
    ])
...
describe('appendHistory', () => {
  ...
  it('trims the head past the cap and advances startDay by what it dropped', () => {
    ...
    const rolled = appendHistory(
      full,
      { [key]: 500 },
      t0 + DAY_MS * HISTORY_CAP_DAYS
    )
...
describe('downsampleByBucketMin', () => {
  ...
  it('takes the minimum of each bucket', () => {
    ...
    expect(out[1].priceSatang).toBe(
      Math.min(...secondBucket.map((p) => p.priceSatang))
    )
  })
})
...
describe('reviveCatalog', () => {
        ...
        editions: {
          standard: {
            ...
            offers: [
              {
                storeId: 'fanatical',
                advertisedSatang: 99132,
                feesSatang: 2478,
                finalSatang: 101610,
                region: 'global',
                drm: 'Steam Key',
                updatedAt: '2026-06-11T20:50:49.000Z',
                isHistoricalLow: false,
                sourceCurrency: 'USD',
                approximate: true,
                voucherCode: null,
                purchaseUrl: 'https://example.com/buy?ref=itad'
              }
            ]
          }
        }
  ...
  it('reports freshness from when we fetched, not when the shop changed its price', () => {
    ...
    expect(
      Date.now() -
        Date.parse(
          catalog.games['test-game'].editions.standard.offers[0].updatedAt
        )
    ).toBeGreaterThan(0)
  ...
  it('rebuilds the fee breakdown so the final price stays internally consistent', () => {
    const snapshot = snapshotFromCatalog(
      catalog,
      'test-game',
      'standard',
  ...
  it('turns stale once the catalog ages past the refresh window', () => {
    ...
    expect(
      snapshotFromCatalog(catalog, 'test-game', 'standard', old).stale
    ).toBe(true)
  })
