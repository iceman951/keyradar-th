...
describe('presentation and trust order', () => {
  it('orders Steam, official stores alphabetically, resellers, then marketplaces', () => {
    ...
    expect(sorted.map((store) => STORE_TRUST_RANK[store.type])).toEqual(
      [...sorted].map((store) => STORE_TRUST_RANK[store.type]).sort((a, b) => a - b)
    );
    const officialNames = sorted.filter((store) => store.type === 'official').map((store) => store.name);
    expect(officialNames).toEqual(
      [...officialNames].sort((a, b) => a.localeCompare(b, 'en', { sensitivity: 'base' }))
    );
  });
  ...
  it('provides an exhaustive presentation for every region', () => {
    const codes: RegionCode[] = ['global', 'sea', 'thailand', 'eu', 'row', 'north-america'];
