...
describe('offer filters', () => {
  ...
  it('applies every boolean offer filter', async () => {
    ...
    const checks: Array<{
      ...
      {
        filters: { thailandOnly: true },
        predicate: (offer) => offer.regionStatus === 'confirmed'
      },
      {
        filters: { steamOnly: true },
        predicate: (offer) => offer.drm.toLowerCase().includes('steam')
      },
      {
        filters: { officialOnly: true },
        predicate: (_offer, store) =>
          store?.type === 'official' || store?.type === 'steam'
      },
      {
        filters: { excludeMarketplace: true },
        predicate: (_offer, store) => store?.type !== 'marketplace'
      },
      { filters: { inStockOnly: true }, predicate: (offer) => offer.inStock },
      {
        filters: { historicalLowOnly: true },
        predicate: (offer) => offer.isHistoricalLow
      },
      {
        filters: { noAdditionalFeeOnly: true },
        predicate: (offer) => offer.fees.length === 0
      }
    ]
    const storesById = new Map(stores.map((store) => [store.id, store]))

    for (const check of checks) {
      const filtered = filterOffers(snapshot.offers, stores, {
        ...permissiveFilters(),
        ...check.filters
      })
      expect(filtered.length).toBeGreaterThan(0)
      expect(
        filtered.every((offer) =>
          check.predicate(offer, storesById.get(offer.storeId))
        )
      ).toBe(true)
    }
  })
  ...
  it('applies maximum price, minimum discount, and edition category filters', async () => {
    ...
    expect(byPrice.every((offer) => offer.finalSatang <= maxPriceSatang)).toBe(
      true
    )
    ...
    const mixedEditions = [
      snapshot.offers[0],
      {
        ...snapshot.offers[1],
        editionKey: 'deluxe',
        editionCategory: 'deluxe' as const
      }
    ]
  ...
  it('filters all candidates before choosing a displayed offer', () => {
    ...
    expect(
      selectDisplayedOffer(candidates, [marketplace, official], {
        ...permissiveFilters(),
