...
const clientRendererUrl = pathToFileURL(
  resolve(
    process.cwd(),
    'node_modules/.pnpm/svelte@5.56.8/node_modules/svelte/src/internal/client/render.js'
  )
).href
const clientRenderer: unknown = await import(
  /* @vite-ignore */ clientRendererUrl
)
...
describe('EmptyState', () => {
  const states: Array<[EmptyStateKind, string, string]> = [
    ...
    [
      'no-th',
      'ยังไม่พบข้อเสนอที่ยืนยันว่าเปิดใช้งานในประเทศไทยได้',
      'ดูข้อเสนอทั้งหมดพร้อมคำเตือน'
    ],
  ...
  it.each(states)(
    'renders the %s state and its action label',
    (kind, title, cta) => {
      const action = vi.fn()
...
describe('OfferCard', () => {
  ...
  it('marks only a confirmed in-stock best offer', () => {
    ...
    const card = loadBody(output.body).querySelector<HTMLElement>(
  ...
  it('renders expanded itemized price and seller details', () => {
    ...
    const detailsButton = card?.querySelector<HTMLButtonElement>(
...
describe('ExitDialog', () => {
  it('renders an accessible dialog with the offer purchase URL and closes on Escape', () => {
    ...
    const onclose = vi.fn()
    const output = render(ExitDialog, {
      props: { offer, game, store, onclose }
    })
    const body = loadBody(output.body)
  ...
  it('focuses the first action, traps focus, and restores the prior focus', async () => {
    ...
    const output = render(ExitDialog, {
      props: { offer, game, store, onclose: vi.fn() }
    })
    const cancel = document.querySelector<HTMLButtonElement>(
      '.dialog footer button'
    )
    const continueLink =
      document.querySelector<HTMLAnchorElement>('.dialog footer a')
