...
import type { GameDto, OfferSnapshotDto, StoreDto } from '../../shared/contracts/api-dto';
...
const errorEnvelope = (code: string, message = 'nope') =>
  jsonResponse({ error: { code, message } }, code === 'VALIDATION_ERROR' ? 400 : 404);
...
const sampleGameDto: GameDto = {
  ...
  popularity: 92,
  hue: 188,
  editions: [{ key: 'standard', name: 'Standard Edition', category: 'standard', steamPriceSatang: 41_500 }]
};
...
describe('ApiGameRepository', () => {
  ...
  it('URL-encodes slugs, queries, and edition keys', async () => {
    ...
    expect(fetchMock).toHaveBeenLastCalledWith('/api/v1/games/valheim/offers?editionKey=gold%20edition');

    fetchMock.mockResolvedValueOnce(jsonResponse([sampleGameDto]));
    await repo.searchGames('el den ring');
    expect(fetchMock).toHaveBeenLastCalledWith('/api/v1/search?q=el%20den%20ring');
  });
  ...
  it('retries against the default edition on EDITION_NOT_FOUND (mock parity)', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(1, '/api/v1/games/valheim/offers?editionKey=unknown-edition');
