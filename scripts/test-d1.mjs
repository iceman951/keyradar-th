...
run('node_modules/.bin/wrangler', ['d1', 'migrations', 'apply', DB_NAME, '--local']);
...
assert(countsAfterFirstSeed.price_history > 0, 'price_history table is populated');

console.log('4. Verifying offers join to editions and stores with no orphans...');
...
console.log('5. Verifying price history is chronological per (game, edition)...');
...
console.log('6. Verifying the final-price invariant holds for every seeded offer...');
const badOffers = query(
  `SELECT id, advertised_satang, final_satang, fees_json FROM offers_current`
).filter((row) => {
  const fees = JSON.parse(row.fees_json).reduce((sum, fee) => sum + fee.amountSatang, 0);
  return row.advertised_satang + fees !== row.final_satang;
});
assert(badOffers.length === 0, 'every offer satisfies finalSatang === advertisedSatang + fees');
...
assert(
  JSON.stringify(countsAfterSecondSeed) === JSON.stringify(countsAfterFirstSeed),
