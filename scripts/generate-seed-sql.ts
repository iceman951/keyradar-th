...
import type { Edition, Game, Offer, OfferFee, Store } from '../shared/domain/models.ts';

const SEED_NOW = process.env.SEED_NOW_MS ? Number(process.env.SEED_NOW_MS) : Date.now();
if (!Number.isFinite(SEED_NOW)) {
  throw new Error(`SEED_NOW_MS must be a finite number, got "${process.env.SEED_NOW_MS}"`);
}
...
const EDITION_CATEGORIES = new Set(['standard', 'deluxe', 'complete', 'dlc', 'bundle']);
const STORE_TYPES = new Set(['steam', 'official', 'reseller', 'marketplace']);
const REGION_CODES = new Set(['global', 'sea', 'thailand', 'eu', 'row', 'north-america']);
...
const validateCatalog = (): void => {
  ...
  for (const game of games) {
    ...
    for (const edition of game.editions) {
      ...
      if (!EDITION_CATEGORIES.has(edition.category)) {
        fail(`unknown edition category "${edition.category}" on ${game.slug}:${edition.key}`);
      }
      if (!Number.isInteger(edition.steamPriceSatang) || edition.steamPriceSatang < 0) {
        fail(`edition ${game.slug}:${edition.key} has a non-integer or negative steamPriceSatang`);
      }
    }
  }
  ...
  for (const store of stores) {
    ...
    if (!STORE_TYPES.has(store.type)) fail(`unknown store type "${store.type}" on ${store.id}`);
    if (!isHttpsUrl(store.websiteUrl)) fail(`store ${store.id} websiteUrl is not HTTPS`);
...
const validateOffer = (offer: Offer, context: string): void => {
  ...
  const feeTotal = offer.fees.reduce((sum: number, fee: OfferFee) => {
    if (!FEE_KINDS.has(fee.kind)) fail(`${context}: unknown fee kind "${fee.kind}"`);
  ...
  if (!REGION_CODES.has(offer.region)) fail(`${context}: unknown region "${offer.region}"`);
  ...
  if (offer.regionStatus !== regionStatusFor(offer.region)) {
    fail(`${context}: regionStatus does not match the centralized region mapping`);
  }
  if (!isHttpsUrl(offer.purchaseUrl)) fail(`${context}: purchaseUrl is not HTTPS`);
  ...
  if (hasRating !== hasReviewCount) {
    fail(`${context}: sellerRating and sellerReviewCount must both be present or both absent`);
  }
};
...
const sqlNumber = (value: number): string => {
  if (!Number.isFinite(value)) fail(`attempted to write a non-finite number: ${value}`);
  return String(value);
};
...
const upsertEdition = (game: Game, edition: Edition, position: number): void => {
  ...
  statements.push(
    `INSERT INTO editions (id, game_slug, edition_key, name, category, steam_price_satang, position, created_at, updated_at) VALUES (${[
      ...
      sqlNumber(SEED_NOW),
      sqlNumber(SEED_NOW)
    ].join(', ')}) ON CONFLICT(id) DO UPDATE SET name=excluded.name, category=excluded.category, steam_price_satang=excluded.steam_price_satang, position=excluded.position, updated_at=excluded.updated_at;`
  );
};
...
const upsertStore = (store: Store): void => {
  ...
  statements.push(
    `INSERT INTO stores (id, name, initials, type, payments_json, fee_rate_bps, fee_label, note, website_url, created_at, updated_at) VALUES (${[
      ...
      sqlNumber(SEED_NOW),
      sqlNumber(SEED_NOW)
    ].join(', ')}) ON CONFLICT(id) DO UPDATE SET name=excluded.name, initials=excluded.initials, type=excluded.type, payments_json=excluded.payments_json, fee_rate_bps=excluded.fee_rate_bps, fee_label=excluded.fee_label, note=excluded.note, website_url=excluded.website_url, updated_at=excluded.updated_at;`
  );
};
...
const insertPricePoint = (
  ...
  if (!Number.isInteger(priceSatang) || priceSatang < 0) {
    fail(`price history point for ${gameSlug} has a non-integer or negative price`);
  }
  statements.push(
...
const build = (): void => {
  ...
  for (const game of games) {
    ...
    for (const [position, edition] of game.editions.entries()) {
      ...
      if (seenEditionIds.has(editionId)) fail(`duplicate edition id "${editionId}"`);
      ...
      if (edition.key === game.editions[0].key) {
        const ending = bestThaiOffer(offers)?.finalSatang ?? edition.steamPriceSatang;
        ...
        for (const point of points) {
          insertPricePoint(game.slug, editionId, point.observedAt, point.priceSatang);
        }
      }
    }
  }
  ...
  for (const game of games) {
    for (const edition of game.editions) {
      const editionId = `${game.slug}:${edition.key}`;
      if (!seenEditionIds.has(editionId)) fail(`FK check: missing edition "${editionId}"`);
    }
  }
  for (const store of stores) {
...
const outPath = join(dirname(fileURLToPath(import.meta.url)), '.generated', 'seed.sql');
