...
test('home discovery, keyboard search, and deal tabs work', async ({
  page
}, testInfo) => {
  await page.goto('/')
  await expect(
    page.getByRole('heading', { name: /ค้นหาราคาเกม Steam/ })
  ).toBeVisible()
  await expect(page.getByTestId('game-card').first()).toBeVisible()

  const search = page
    .locator('.hero-search')
    .getByRole('textbox', { name: 'ค้นหาเกม' })
  await search.fill('Valheim')
  await expect(
    page.getByRole('listbox', { name: 'ผลการค้นหาเกม' })
  ).toBeVisible()
  ...
  if (testInfo.project.name === 'mobile') {
    ...
    await page
      .getByRole('link', { name: 'ร้านค้า', exact: true })
      .first()
      .click()
...
test('search filters apply before offer selection and sorting remains functional', async ({
  page
}, testInfo) => {
  await page.goto('/search?q=Forest')
  await expect(page.getByRole('heading', { name: /ผลการค้นหา/ })).toBeVisible()
  await expect(
    page.getByText('The Forest', { exact: true }).first()
  ).toBeVisible()
  ...
  if (testInfo.project.name === 'mobile') {
    ...
    await sheet
      .getByRole('checkbox', { name: /ไม่มีค่าธรรมเนียมเพิ่มเติม/ })
      .check()
    await sheet.getByRole('button', { name: 'Standard', exact: true }).click()
    await sheet.getByRole('button', { name: /ดูผลลัพธ์/ }).click()
    await expect(
      page.getByText('ไม่มีค่าธรรมเนียมเพิ่ม', { exact: true })
    ).toBeVisible()
    await page.getByRole('button', { name: /ราคาต่ำสุด/ }).click()
    await page
      .getByRole('dialog', { name: 'เรียงตาม' })
      .getByRole('button', { name: /อัปเดตล่าสุด/ })
      .click()
  } else {
...
test('game editions, retained refresh data, and stale warnings are represented', async ({
  page
}) => {
  ...
  await expect(
    page.getByText(/ถูกที่สุดที่ใช้งานในไทยได้/).first()
  ).toBeVisible()
...
test('uncertain-region, no-Thai, no-price, and out-of-stock states have recovery UI', async ({
  page
}) => {
  ...
  await expect(
    page
      .getByText('ตรวจสอบภูมิภาค', { exact: true })
      .filter({ visible: true })
      .first()
  ).toBeVisible()
  ...
  await expect(
    page.getByRole('heading', { name: /ยังไม่พบข้อเสนอที่ยืนยัน/ })
  ).toBeVisible()
  await page
    .getByRole('button', { name: 'ดูข้อเสนอทั้งหมดพร้อมคำเตือน' })
    .click()
  await expect(
    page
      .getByText('ตรวจสอบภูมิภาค', { exact: true })
  ...
  await expect(
    page.getByRole('heading', { name: 'ยังไม่มีข้อมูลราคาสำหรับเกมนี้' })
  ).toBeVisible()
  await expect(
    page.getByRole('button', { name: 'ลองดึงราคาอีกครั้ง' })
  ).toBeVisible()
  ...
  await expect(
    page
      .getByText('สินค้าหมด', { exact: true })
      .filter({ visible: true })
      .first()
  ).toBeVisible()
})

test('outbound confirmation is accessible and the mobile CTA remains viewport-fixed', async ({
  page
}, testInfo) => {
  await page.goto('/games/elden-ring')
  await waitForPrices(page)
  ...
  if (testInfo.project.name === 'mobile') {
    ...
    await page
      .locator('.best-foot')
      .getByRole('button', { name: /ไปยังร้านค้า/ })
      .click()
  }

  const dialog = page.getByRole('dialog', {
    name: 'คุณกำลังออกจาก KeyRadar TH'
  })
  ...
  if (testInfo.project.name === 'mobile')
    await expect(page.locator('.sticky-buy')).toBeHidden()
  await page.keyboard.press('Escape')
  await expect(dialog).toBeHidden()
  if (testInfo.project.name === 'mobile')
    await expect(page.locator('.sticky-buy')).toBeVisible()
})
...
test('reference-aligned Svelte screen baseline', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.hero')).toHaveScreenshot(
    `home-hero-${testInfo.project.name}.png`,
    {
      animations: 'disabled'
    }
  )
