...
export const createPricingService = (
  ...
  return {
    async getOffers(gameSlug, editionKey) {
      ...
      const offers = rows.map(({ offer, edition: editionRow }) =>
        offerRowToDto(offer, editionRow)
      )
    ...
    async getEditionAvailability(gameSlug) {
      ...
      const aggregates =
        await repository.getEditionAvailabilityAggregates(gameSlug)
      return aggregates.map((aggregate) =>
        editionAvailabilityDto(aggregate.edition, aggregate)
      )
    },
    ...
    async getPriceHistory(gameSlug, days) {
      ...
      const rows = await repository.getPriceHistory(
        gameSlug,
        editionId,
        sinceMs
      )
      return rows.map((row): PricePointDto => ({
