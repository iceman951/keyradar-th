...
export interface PricingRepository {
   ...
   */
  getEditionAvailabilityAggregates(gameSlug: string): Promise<EditionAvailabilityAggregate[]>;
...
export const createPricingRepository = (db: Database): PricingRepository => ({
  async getOffersForEdition(gameSlug, editionId) {
    return db
      ...
      .where(and(eq(offersCurrent.gameSlug, gameSlug), eq(offersCurrent.editionId, editionId)))
      .orderBy(
  ...
  async getEditionAvailabilityAggregates(gameSlug) {
    ...
    const rows = await db
      .select({
        ...
        minimumPriceSatang: sql<number | null>`MIN(CASE WHEN ${confirmedInStock} THEN ${offersCurrent.finalSatang} END)`
      })
      .from(editions)
    ...
    return rows.map((row) => ({
      ...
      minimumPriceSatang: row.minimumPriceSatang === null ? null : Number(row.minimumPriceSatang)
    }));
  },
  ...
  async getPriceHistory(gameSlug, editionId, sinceMs) {
    return db
      .select({ observedAt: priceHistory.observedAt, priceSatang: priceHistory.priceSatang })
      .from(priceHistory)
      .where(
        and(
