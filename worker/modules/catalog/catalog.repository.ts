...
export interface CatalogRepository {
  ...
  searchGames(query: string, limit?: number): Promise<{ game: GameRow; editionRows: EditionRow[] }[]>;
  getGame(slug: string): Promise<{ game: GameRow; editionRows: EditionRow[] } | undefined>;
...
export const createCatalogRepository = (db: Database): CatalogRepository => ({
  async listGames({ limit, sort = 'popular' }) {
    const query = db.select().from(games).orderBy(...orderForSort(sort));
    const gameRows = limit ? await query.limit(limit) : await query;
    const editionsBySlug = await editionsByGameSlug(db, gameRows.map((game) => game.slug));
    return gameRows.map((game) => ({ game, editionRows: editionsBySlug.get(game.slug) ?? [] }));
  },
