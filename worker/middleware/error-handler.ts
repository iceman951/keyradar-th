 ...
 */
export const errorHandler = ({ error, set }: ErrorHandlerContext): ApiErrorDto => {
  ...
  if (error instanceof ValidationError) {
    set.status = 400;
    return { error: { code: 'VALIDATION_ERROR', message: 'The request did not pass validation.' } };
  }

  // No route matched the path at all (e.g. GET /api/v1/unknown).
  if (error instanceof NotFound) {
    set.status = 404;
    return { error: { code: 'NOT_FOUND', message: 'No route matches this path.' } };
  }
