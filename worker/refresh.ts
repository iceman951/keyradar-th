...
import { buildCatalog, editionLookups, lookupKey } from '../src/lib/itad/build-catalog';
import { crossRates } from '../src/lib/itad/fx';
import { appendHistory, historyKey, ictEpochDay } from '../src/lib/itad/history';
import { parseSteamPrices, steamPricesUrl, type SteamPrice } from '../src/lib/itad/steam';
import {
  ID_CACHE_VERSION,
  type Catalog,
  type CatalogFx,
...
const FX_URL = 'https://api.frankfurter.dev/v1/latest?base=USD&symbols=THB,EUR,GBP,AUD,CAD,PLN,BRL,RUB';
...
const fetchPrices = async (
  ...
  if (!response.ok) {
    log({ evt: 'prices_failed', status: response.status, body: await response.text() });
    return null;
  }
...
export const refreshCatalog = async (env: RefreshEnv, at: number): Promise<RefreshResult> => {
  if (!env.ITAD_API_KEY) {
  ...
  const titles = [...new Set(lookups.filter((l) => !l.itadId).map((l) => l.lookupTitle))];
  ...
  const { cache, requested } = await resolveIds(cached ?? emptyIdCache(), titles, today);
  ...
  const steamAppIds = [
    ...new Set(
      games.flatMap((game) => [
        ...
        ...game.editions.flatMap((edition) => (edition.steamAppId ? [edition.steamAppId] : []))
      ])
    )
  ];

  const [fx, steamPrices] = await Promise.all([fetchFx(previous), fetchSteamPrices(steamAppIds)]);
  ...
  const offerCount = Object.values(catalog.games).reduce(
    (total, game) =>
      total + Object.values(game.editions).reduce((sum, e) => sum + e.offers.length, 0),
    0
  );
