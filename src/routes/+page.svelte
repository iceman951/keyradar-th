...
<main>
  <section class="hero">
    ...
    <div class="page hero-inner">
      <div class="live">
        <b><i></i> LIVE</b> ตรวจสอบราคาจาก {stores.length || '—'} ร้านค้า · อัปเดตทุก
        8 ชั่วโมง
      </div>
      ...
      <div class="popular">
        <span>ยอดค้นหา</span>
        {#each popular as name}<a href={`/search?q=${encodeURIComponent(name)}`}
            >{name}</a
          >{/each}
      </div>
      <div class="trust">
        {#each trustPoints as point}<span
            ><Icon name="check" size={15} />{point}</span
          >{/each}
      </div>
    </div>
  </section>
  ...
  <section class="page">
    <header class="section-head">
      <div>
        ...
        <p>
          คัดเฉพาะดีลที่มีอย่างน้อยหนึ่งข้อเสนอที่ยืนยันว่าเปิดใช้งานในไทยได้
        </p>
      </div>
      <a class="btn" href="/deals">ดูทั้งหมด</a>
    </header>
    <div class="cards">
      {#each cards as card}<GameCard {...card} />{/each}
    </div>
  </section>
  ...
  <section class="page" id="how">
    <div class="how">
      <div class="intro">
        ...
        <p>
          KeyRadar TH ไม่ได้ขายเกมเอง
          เรารวบรวมราคาจากร้านค้าปลายทางแล้วคำนวณราคาสุทธิโดยประมาณให้เปรียบเทียบได้ในที่เดียว
        </p>
      </div>
      <div class="steps">
        {#each steps as step, index}
          <article class="surface">
            <div>
              <b><Icon name={step.icon} size={17} /></b><small
                >ขั้นตอนที่ {index + 1}</small
              >
            </div>
            <h3>{step.title}</h3>
            <p>{step.body}</p>
  ...
  <section class="page">
    <header class="section-head">
      <div>
        ...
        <p>
          เราแยกประเภทร้านค้าอย่างชัดเจน — ร้านค้าอย่างเป็นทางการไม่เหมือน
          Marketplace
        </p>
      </div>
    ...
    <div class="store-grid">
      {#each stores as store}
        <a class="surface" href="/stores">
          ...
          <span
            ><b>{store.name}</b><small>{storeTypeLabel(store.type)}</small
            ></span
          >
        </a>
      {/each}
...
<style>
  ...
  .hero-decor {
    ...
    background:
      ...
      radial-gradient(80% 60% at 92% 8%, rgb(90 180 150 / 10%), transparent 60%);
  }
  .hero-inner {
    position: relative;
    max-width: 1320px;
  ...
  .radar {
    ...
    width: 460px;
    height: 460px;
    border: 1px solid rgb(145 132 217 / 20%);
    border-radius: 50%;
    opacity: 0.9;
  }
  .radar i {
    position: absolute;
    inset: 18%;
    border: 1px solid rgb(145 132 217 / 16%);
    border-radius: 50%;
  }
  .radar i:nth-child(2) {
    inset: 36%;
