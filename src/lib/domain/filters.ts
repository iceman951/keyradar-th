...
import { compareOffersByFinalPrice, discountPercent, hasAdditionalFees } from './pricing';
...
export const offerMatchesFilters = (
  ...
  if (filters.officialOnly && store?.type !== 'official' && store?.type !== 'steam') return false;
  ...
  if (filters.editionCategory !== 'all' && offer.editionCategory !== filters.editionCategory) {
    return false;
  }
  return true;
};
...
export const filterOffers = (
  ...
  return offers
    .filter((offer) => offerMatchesFilters(offer, storesById.get(offer.storeId), filters))
