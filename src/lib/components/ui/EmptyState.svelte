  ...
  const content = $derived.by(
    (): EmptyContent =>
      (
        ...
        }) satisfies Record<EmptyStateKind, EmptyContent>
      )[kind]
  )
</script>
...
<section class="empty surface" data-testid="empty-state" aria-live="polite">
  <span
    class="icon"
    class:warning={content.tone === 'warning'}
    class:error={content.tone === 'error'}
  >
    <Icon name={content.icon} size={25} />
  </span>
  <h2>{content.title}</h2>
  <p>{content.body}</p>
  {#if content.tips}<div class="tips">
      {#each content.tips as tip}<span>{tip}</span>{/each}
    </div>{/if}
  {#if action}<button type="button" class="btn btn-accent" onclick={action}
      >{content.cta}</button
    >{/if}
</section>
