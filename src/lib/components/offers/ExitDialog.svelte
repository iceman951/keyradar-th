<script lang="ts">
  ...
  $effect(() => {
    ...
    void tick().then(() =>
      dialogElement?.querySelector<HTMLElement>('button, a[href]')?.focus()
    )
    return () => {
...
>
  ...
  >
    <header>
      ...
      <div>
        <h2 id="dialog-title">คุณกำลังออกจาก KeyRadar TH</h2>
        <p id="dialog-description">
          เรากำลังพาคุณไปที่ <strong>{store.name}</strong> เพื่อทำรายการซื้อ การชำระเงินและการรับคีย์ทั้งหมดเกิดขึ้นบนเว็บไซต์ของร้านค้า
        </p>
      </div>
    </header>
    ...
    <div class="rows">
      ...
      <p>
        <span>ค่าธรรมเนียมที่ทราบ</span><b
          >{feeTotal ? `+ ${formatBaht(feeTotal)}` : 'ไม่มี'}</b
        >
      </p>
      <p class="total" class:confirmed={offer.regionStatus === 'confirmed'}>
        <span>ราคาสุทธิโดยประมาณ</span><b
          >{offer.approximate ? '~' : ''}{formatBaht(offer.finalSatang)}</b
        >
      </p>
    ...
    {#if offer.voucherCode}
      <div class="warning uncertain">
        <Icon name="warning" size={15} /><span
          >ราคานี้ต้องใช้โค้ดส่วนลด <strong>{offer.voucherCode}</strong> ที่หน้าชำระเงิน
          หากไม่ใส่โค้ด คุณจะถูกคิดราคาเต็ม</span
        >
      </div>
    {/if}
    ...
    {#if offer.approximate}
      <div class="warning">
        <Icon name="warning" size={15} /><span
          >ร้านนี้คิดราคาเป็นสกุล <strong>{offer.sourceCurrency}</strong> ยอดข้างต้นเป็นการแปลงเป็นเงินบาทโดยประมาณ
          รวมค่าธรรมเนียมแปลงสกุลเงินที่ประเมินไว้ ยอดจริงที่ถูกเรียกเก็บขึ้นกับอัตราแลกเปลี่ยนของผู้ออกบัตรคุณ</span
        >
      </div>
    {/if}
    ...
    >
      <Icon
        name={offer.regionStatus === 'blocked' ? 'x' : 'warning'}
        size={15}
      /><span>{warning}</span>
    </div>
    ...
    <footer>
      ...
      <a
        class="btn continue"
        class:confirmed={offer.regionStatus === 'confirmed'}
        class:blocked={offer.regionStatus === 'blocked'}
        href={offer.purchaseUrl}
        target="_blank"
        rel="noopener noreferrer"
        onclick={onclose}
        >ไปยัง {store.name} <Icon name="external" size={14} /></a
      >
    </footer>
    ...
    <p class="attribution">
      ข้อมูลราคารวบรวมผ่าน <a
        href="https://isthereanydeal.com/"
        target="_blank"
        rel="noopener noreferrer">IsThereAnyDeal</a
      >
    </p>
  </div>
</div>
...
<style>
  ...
  .dialog {
    ...
    box-shadow: 0 28px 70px rgb(0 0 0 / 66%);
    animation: rise 0.18s;
  }
  .dialog header {
    display: flex;
  ...
  .dialog footer .continue.confirmed {
    border-color: var(--ok-line);
    background: var(--ok-dim);
    color: var(--ok);
  }
  .dialog footer .continue.blocked {
    border-color: var(--bad-line);
    background: var(--bad-dim);
    color: var(--bad);
  }
