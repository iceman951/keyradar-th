    ...
    {#if store}
      <div class="offer" class:best={isBest} class:blocked={offer.regionStatus === 'blocked'} role="rowgroup">
        <div class="row" role="row">
          ...
          <span class="region" role="cell"><i class="tag" class:tag-warn={offer.regionStatus === 'uncertain'} class:tag-bad={offer.regionStatus === 'blocked'}>{region.label}</i><i class="tag" class:tag-ok={offer.regionStatus === 'confirmed'} class:tag-warn={offer.regionStatus === 'uncertain'} class:tag-bad={offer.regionStatus === 'blocked'}>{regionStatusLabel(offer.regionStatus)}</i></span>
          ...
          <span class="number final" class:confirmed-best={isBest} role="cell"><strong>{formatBaht(offer.finalSatang)}</strong>{#if discountPercent(offer)}<small class:confirmed-best={isBest}>−{discountPercent(offer)}%</small>{/if}</span>
        ...
        {#if expanded === offer.id}
          <div class="detail" id={detailId(offer)} role="row">
            ...
            <section role="cell"><h4>ผู้ขายและความน่าเชื่อถือ</h4>{#if offer.sellerRating !== undefined}<p><span>คะแนนผู้ขาย</span><b>{offer.sellerRating.toFixed(1)}%</b></p>{/if}{#if offer.sellerReviewCount !== undefined}<p><span>จำนวนรีวิวผู้ขาย</span><b>{offer.sellerReviewCount.toLocaleString('th-TH')}</b></p>{/if}<p><span>ประเภทร้าน</span><b>{storeTypeLabel(store.type)}</b></p><p><span>การชำระเงิน</span><b>{store.payments.slice(0, 2).join(' · ')}</b></p></section>
