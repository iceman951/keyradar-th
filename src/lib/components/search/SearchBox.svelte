<script lang="ts">
  ...
  const listboxId = $derived(
    big
      ? 'hero-game-search-results'
      : compact
        ? 'header-game-search-results'
        : 'game-search-results'
  )
  const inputPlaceholder = $derived(
    placeholder ??
      (big
        ? mobile
          ? 'ค้นหาเกม เช่น Elden Ring'
          : 'ค้นหาเกม เช่น Valheim, Elden Ring, Palworld'
        : 'ค้นหาเกม…')
  )
...
<div class="wrap" class:big class:compact>
  ...
  >
    ...
    <input
      ...
      aria-activedescendant={focused && activeIndex >= 0 && matches[activeIndex]
    ...
    {/if}
    {#if !compact}<button class="submit" type="submit"
        >{mobile && big ? 'ค้นหา' : 'ค้นหาเกม'}</button
      >{/if}
  </form>
  {#if focused}
    <div class="dropdown">
...
<style>
  ...
  form {
    ...
    border-radius: 15px;
    background: linear-gradient(180deg, #1d2030, #191c2a);
  ...
  .compact form:focus-within {
    box-shadow: none;
  }
  .compact form > :global(svg) {
    color: var(--mute2);
  }
  .compact input {
    font-size: 13.5px;
