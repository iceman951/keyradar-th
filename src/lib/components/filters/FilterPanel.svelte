<script lang="ts">
  ...
  const setEdition = (editionCategory: EditionCategory | 'all') => {
    filters = { ...filters, editionCategory }
  }

  const groups: {
    heading: string
    items: readonly [ToggleKey, string, string][]
  }[] = [
    {
      ...
      items: [
        [
          'thailandOnly',
          'ใช้งานได้ในประเทศไทย',
          'แสดงเฉพาะข้อเสนอที่ยืนยันภูมิภาคแล้ว'
        ],
        [
          'steamOnly',
          'เปิดใช้งานผ่าน Steam',
          'ไม่รวมคีย์ Epic / Ubisoft / EA App'
        ]
      ]
    },
    {
      ...
      items: [
        [
          'officialOnly',
          'เฉพาะร้านค้าอย่างเป็นทางการ',
          'ตัวแทนจำหน่ายที่ได้รับอนุญาตเท่านั้น'
        ],
        [
          'excludeMarketplace',
          'ไม่รวม Marketplace',
          'ตัดผู้ขายรายย่อยที่มีค่าธรรมเนียมเพิ่ม'
        ]
      ]
    },
    {
...
<section class="panel" class:bare aria-label="ตัวกรองราคาเกม">
  ...
  <label class="range">
    <span
      >ช่วงราคา <b
        >ไม่เกิน ฿{Math.round(filters.maxPriceSatang / 100).toLocaleString(
          'th-TH'
        )}</b
      ></span
    >
    <input
      ...
      max="240000"
      step="5000"
      bind:value={filters.maxPriceSatang}
    />
    ...
    <span
      >ส่วนลดขั้นต่ำ <b
        >{filters.minDiscountPercent
          ? `ตั้งแต่ ${filters.minDiscountPercent}%`
          : 'ทั้งหมด'}</b
      ></span
    >
    <input
      aria-label="ส่วนลดขั้นต่ำ"
      type="range"
      min="0"
      max="80"
      step="5"
      bind:value={filters.minDiscountPercent}
    />
  </label>
    ...
    <fieldset class="checks">
      ...
      {#each group.items as row}
        <label class:active={filters[row[0]]}>
          <input
            type="checkbox"
            checked={filters[row[0]]}
            onchange={() => toggle(row[0])}
          />
          <span
            >{row[1]}{#if row[2]}<small>{row[2]}</small>{/if}</span
          >
        </label>
      {/each}
    </fieldset>
    ...
    <div>
      {#each editions as edition}
        <button
          ...
          onclick={() => setEdition(edition[0])}>{edition[1]}</button
        >
      {/each}
    </div>
  </fieldset>
