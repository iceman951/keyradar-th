...
export const ictEpochDay = (at: number): number =>
  Math.floor((at + ICT_OFFSET_MS) / DAY_MS)

export const epochDayToDate = (day: number): Date =>
  new Date(day * DAY_MS - ICT_OFFSET_MS)
...
export const appendHistory = (
  ...
  if (overflow > 0) {
    for (const key of Object.keys(series))
      series[key] = series[key].slice(overflow)
  }

  return {
    version: HISTORY_VERSION,
    startDay: base.startDay + overflow,
    series
  }
}
...
export const downsampleByBucketMin = (
  ...
  for (let i = 0; i < limit; i += 1) {
    ...
    const end = Math.min(
      points.length,
      Math.max(start + 1, Math.floor((i + 1) * size))
    )
