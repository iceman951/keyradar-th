...
export const drmLabel = (deal: ItadDeal, storeId: string): string => {
  ...
  const names = (deal.drm ?? []).map((entry) =>
    entry.name.toLocaleLowerCase('en')
  )
  if (names.some((name) => name.includes('gog') || name.includes('drm free')))
    return 'GOG.com'
  if (names.some((name) => name.includes('ea ') || name.includes('origin')))
    return 'EA app'
  if (names.some((name) => name.includes('epic'))) return 'Epic Games'
  if (
    names.some((name) => name.includes('ubisoft') || name.includes('uplay'))
  ) {
...
export const mapDealToCatalogOffer = ({
  ...
  const feesSatang = approximate
    ? Math.round(advertisedSatang * FX_FEE_RATE)
    : 0
  ...
  return {
    ...
    isHistoricalLow:
      ...
      advertisedSatang <= historyLowSatang * HISTORICAL_LOW_TOLERANCE,
