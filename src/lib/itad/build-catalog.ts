...
const steamOfferFrom = (
  ...
  isHistoricalLow:
    ...
    price.finalSatang <= historyLowSatang * HISTORICAL_LOW_TOLERANCE,
  sourceCurrency: 'THB',
  approximate: false,
  voucherCode: null,
...
export const editionLookups = (games: readonly Game[]): EditionLookup[] =>
  games.flatMap((game) =>
    game.editions.map((edition) => ({
      ...
      lookupTitle:
        edition.itadTitle ??
        (edition.key === 'standard'
          ? game.title
          : `${game.title} ${edition.name}`),
...
export const buildCatalog = ({
  ...
  for (const store of stores) {
    if (typeof store.itadShopId === 'number')
      storeByShopId.set(store.itadShopId, store)
  }
  ...
  for (const game of games) {
    ...
    for (const edition of game.editions) {
      const itadId =
        edition.itadId ?? idByEdition[lookupKey(game.slug, edition.key)]
      ...
      const steamAppId =
        edition.steamAppId ??
        (edition.key === 'standard' ? game.steamAppId : undefined)
      ...
      if (steam && steamAppId) {
        offers.push(
          steamOfferFrom(steam, steamAppId, historyLowSatang, fetchedAt)
        )
    ...
    if (carried && Object.keys(carried.editions).length > 0) {
      ...
      for (const carriedEdition of Object.values(carried.editions)) {
        for (const offer of carriedEdition.offers)
          seenStoreIds.add(offer.storeId)
      }
      continue
    }
  ...
  const allowlisted = stores.filter(
    (store) => typeof store.itadShopId === 'number'
  )
  ...
  return {
    ...
    failedStores: allowlisted
      .filter(
        (store) => !seenStoreIds.has(store.id) && previousStoreIds.has(store.id)
      )
      .map((store) => store.id),
    games: catalogGames
  }
