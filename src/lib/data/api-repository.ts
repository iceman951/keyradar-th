...
const throwForFailure = (result: { ok: false; status: number; code?: string }): never => {
...
const toEditionAvailability = (dto: EditionAvailabilityDto): EditionAvailability => ({ ...dto });
...
export class ApiGameRepository implements GameRepository {
  ...
  private async fetchJson<T>(path: string): Promise<FetchResult<T>> {
    ...
    try {
      ...
      throw new ApiRequestError('ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้ กรุณาลองใหม่อีกครั้ง');
    }
  ...
  async getGame(slug: string): Promise<Game | undefined> {
    const result = await this.fetchJson<GameDto>(`/games/${encodeURIComponent(slug)}`);
    if (!result.ok) {
  ...
  async searchGames(query: string): Promise<Game[]> {
    ...
    const dtos = trimmed
      ? await this.request<GameDto[]>(`/search?q=${encodeURIComponent(trimmed)}`)
   ...
   */
  async getOffers(gameSlug: string, editionKey?: string): Promise<OfferSnapshot> {
    const query = editionKey ? `?editionKey=${encodeURIComponent(editionKey)}` : '';
